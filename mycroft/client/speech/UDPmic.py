...
class MutableUDPMicrophone(AudioSource):
    def __init__(self, sample_rate=16000, chunk_size=640, mute=False):
    ...
    def __enter__(self):
        ...
        self.stream = MutableUDPStream(udp, self.format, self.muted)
        return self
...
class ResponsiveRecognizer(speech_recognition.Recognizer):
    ...
    def _wait_until_wake_word(self, source, sec_per_buffer):
        ...
        LOG.info("buffers_per_check" + str(buffers_per_check))  # Max bytes for byte_data before audio is removed from the front
        ...
        if ww_module == 'PreciseHotword':
            ...
            model_hash = '0'
        counter = 0
        LOG.info("said Wake word" + str(said_wake_word))
        LOG.info("Stop Signaled" + str(self._stop_signaled))
        ...
        while not said_wake_word and not self._stop_signaled:
            ...
            # stream.write(byte_data, CHUNK)
            buffers_since_check += 1.0

            self.wake_word_recognizer.update(chunk)
            # LOG.info("buffers_since_check" + str(buffers_since_check))
            ...
            if buffers_since_check > buffers_per_check:
                ...
                said_wake_word = \
                    self.wake_word_recognizer.found_wake_word(byte_data)
                # LOG.info("said Wake word" + str(said_wake_word))
